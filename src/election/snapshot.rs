...
impl Snapshot {
    ...
    pub fn take_snapshot_metadata(
        ...
        info!(
            "success to task snapshot metadata, filepath: {}",
            metadata_filepath
        );
    }
    ...
    // snapshot The latest snapshot path in dir
    pub fn latest_snapshot_filepath(&mut self) -> Option<String> {
        ...
        for item in result {
            ...
            if filename.ends_with(".snapshot") {
                let re = regex::Regex::new(r"election-(\d+)-(\d+).snapshot").unwrap();
    ...
    pub fn latest_metadata_filepath(&mut self) -> Option<String> {
        ...
        for item in result {
            ...
            if filename.ends_with(".snapshot.metadata") {
                let re = regex::Regex::new(r"election-(\d+)-(\d+).snapshot.metadata").unwrap();
                let caps = re.captures(filename).unwrap();
    ...
    pub fn gen_tmp_snapshot_metadata_filepath(
        ...
        format!(
            "{}/election-{}-{}.snapshot.metadata.tmp",
            self.snapshot_dir, last_included_index, last_included_term
        )
    }
}
